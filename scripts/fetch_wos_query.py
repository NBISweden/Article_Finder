...
def extract_records_any(data):
    try:
        ...
        if isinstance(r, dict):
            rec = r.get("REC", [])
            return [rec] if isinstance(rec, dict) else rec
...
def run_fetch_query(
    ...
    with requests.Session() as session:
        ...
        seed_resp = get_with_retry(session, base, params=seed_params)
        query_id, total_found = get_query_id_and_total(seed_resp.json())
        ...
        for first in range(1, total_to_fetch + 1, page_size):
            ...
            if start_date and end_date:
                params["publishTimeSpan"] = f"{start_date}+{end_date}"
            ...
            if not recs:
                print("⚠️ No records extracted on this page. Check debug_first_page.json for schema.")
                break
            ...
            print(f" Progress: {len(summary_rows)}/{total_to_fetch}")
